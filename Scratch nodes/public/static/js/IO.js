...
var IO = function() {
    ...
    IO.prototype.loadNotesDrums(); 
};

var prefixSoundbank = 'static/soundbank/';

IO.prototype.soundRequest = function(sound, sprite) {
    var request = new XMLHttpRequest();
    request.open('GET', sound['md5'] + this.asset_suffix, true);
...
IO.prototype.loadNotesDrums = function() {
    $.each(Instr.wavs, function(name, file) {
        var request = new XMLHttpRequest();
        request.open('GET', prefixSoundbank + file, true);
        request.responseType = 'arraybuffer';
        request.onload = function() {
